...
# $1 : sauvegarde DB 0/1 (1 par defaut)
#----------------------
...
# - ajout de l'instance à un groupe de projets et à un groupe de portfolios (par diplome)
# - partage avec les usagers (role etudiant et enseignant)
#----------------------

# Pre-requis
# Installer xmlstarlet, xz et dos2unix
# disposer des commandes xmllint, curl
...
USR_DB=**user_db**                  # authentification : user mariadb
PWD_DB=**password_db**              # authentification : password mariadb
DBN_DB="karuta-backend"             # nom de la bdd karuta
...
XZ="/usr/bin/xz"
DIR_USERS="./utilisateurs"     # Dossier où sont situés les fichiers csv
...
# Sauvegarde eventuelle de la bdd :
#--------------------------------------
save_db=$1
if test -z $save_db
then
...
then
	...
	# MYSQL_PWD=$PWD_DB $DIR_MARIADB/bin/mysqldump --user $USR_DB --databases $DBN_DB | $XZ -9z >$DIR_DUMP/karuta-backend-import-$FORMATTED_DATE.sql.xz
	MYSQL_PWD=mysqldump --host=$SRV_DB --user=$USR_DB --password=$PWD_DB --databases $DBN_DB | $XZ -9z >$DIR_DUMP/karuta-backend-import-$FORMATTED_DATE.sql.xz 
	debug "-> Sauvegarde DB effectuée !"
fi


# Connexion Root et récupération du cookie :
#-------------------------------------------
debug "-> Connexion"
...
do line="$username $studentid $firstname $lastname $birthdate $email $role $diplome"
	...
	# Recherche de l'id du modèle projet Karuta Racine
	...
	then
		...
		# Modification du label du projet
	...
	then
		...
		then
			...
			debug "-> Recharge la copie de travail : $id_copie_travail"
			...
			# Suppression des anciens noeuds compétences de la formation et autres compétences :
